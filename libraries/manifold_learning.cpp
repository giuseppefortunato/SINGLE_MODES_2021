...
MatrixXd multidim_scaling(MatrixXd& distance, int s, int d, int Rank){
...
void ISOMAP(MatrixXd &sn_set, prob_settings &settings, int Nr, int q, VectorXi &k, VectorXd &kruskal_min, MatrixXd &y, int number_r, VectorXi &Rank){

    MatrixXd sn_set_partial(Nr, sn_set.cols());
    sn_set_partial = sn_set.block(Nr * q, 0, Nr, sn_set.cols());


    for (int i = 0; i < sn_set_partial.rows(); i++) {
        for(int j=0; j < sn_set_partial.cols(); j++)
            sn_set_partial(i,j)= (sn_set_partial(i,j)-sn_set_partial.minCoeff())/(sn_set_partial.maxCoeff()-sn_set_partial.minCoeff());
    ...
    for( int i=0; i < settings.neighbors.size(); i++ ){
        ...
        MatrixXd reduced = multidim_scaling(distance_input,                         // Dimensionality reduction of the system.
                                            ...
                                            settings.r_isomap[number_r],
                                            Rank(q));                                    // reduced (Ns, r_isomap ));
        ...
        if (kruskal_stress(i)<kruskal_min(q)) {
            for (int j = 0; j < settings.r_isomap[number_r]; j++)
                y.row((q * settings.r_isomap[number_r]) + j) = reduced.row(j);
...
void penalty_term(MatrixXd &C, int k_reconstruction, VectorXd &embedding_sorted){
    ...
    double eps= 0.01;  //0.01 in origine
    double omega= 4;   //4 in origine
...
MatrixXd create_rec_fields(prob_settings &settings, std::vector<double> &y_star, Eigen::MatrixXd &y_part,
                           Eigen::MatrixXd &sn_set_partial, int number_r, int f){

    //  NEAREST POINT TO RECONSTRUCTION ON y*
    Eigen::VectorXd embedding_sorted = Eigen::VectorXd::Zero(settings.r_isomap[number_r]);
    ...
    Eigen::MatrixXd C = Eigen::MatrixXd::Zero(settings.k_rec[f], settings.k_rec[f]);
    penalty_term(C, settings.k_rec[f], embedding_sorted);

    //EVALUATION OF GRAM MATRIX
    Eigen::MatrixXd G(settings.k_rec[f], settings.k_rec[f]);
    gram_matrix(G, settings.k_rec[f], y_star_index_nearest, y_part, y_star);

    //EVALUATION OF WEIGHTS
    Eigen::MatrixXd G_tilde = G+C ;
    Eigen::VectorXd weigths;
    weigths = lagrange(G_tilde);
    std::cout<<"vector of weights "<<std::endl<< std::setprecision(12)<<weigths.transpose()<<std::endl<<std::endl;

    Eigen::MatrixXd snapshot_star = Eigen::MatrixXd::Zero(sn_set_partial.rows(), 1);
    for (int index = 0; index < settings.k_rec[f]; index++) {
        int snap = y_star_index_nearest(index);
        snapshot_star = snapshot_star + (weigths(index) * sn_set_partial.col(snap));
    }
