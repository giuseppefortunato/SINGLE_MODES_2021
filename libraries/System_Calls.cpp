...
void SU2_DTR(prob_settings settings,std::string filename, const std::string& su2_conf, int nt, int number_r ){
    ...
    Modify_su2_cfg( su2_conf, su2_conf_new, std::move(filename), settings, nt, number_r);

    //String to launch SU2_DTR from terminal
    std::string su2dtr_string = "./SU2_CFD_residual " + su2_conf_new + " > SU2_" + settings.flag_method[0] + "_" + std::to_string (nt+1) +".log";
    ...
    if (settings.flag_rec== "YES"){
        std::cout << "Calling SU2_CFD for residual evaluation and writing file to history... ";
        std::system(su2_sys_call);
        std::cout <<"done"<<std::endl;
    }else if (settings.flag_rec=="NO"){
        std::cout <<" Calling SU2_SOL for visualization...";
